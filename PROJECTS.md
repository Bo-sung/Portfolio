...
- **결정적 피봇(자소서 확인)**: 초기 기획은 단순 플랫포머 → 프로토타이핑 후 "너무 단순" 판단 → 점프킹을 모티브로 한 발사형 이동으로 전환(결과물은 Getting Over It에 가까움)하며 게임성 개선, 우승. 참가팀 중 유일한 언리얼 사용 팀
...
- **사용자 제공 자소서(2025)** → 추가 확보 정보: 산업기능요원 복무, 후반기 PM 업무(시즌 이벤트·상품 패키지·NFT 펫 리뉴얼), 밤샘 거래버그 픽스 에피소드, 3인 벽돌깨기 프로젝트(5~12세 타겟, SDK 추상화 — `G:\HDD_Backup\Git\BBF_Client`/`MFB_Client` 백업과 연결 추정), 원정대는 "탐사" 시스템, Creavas는 플랫포머→점프킹 모티브 발사형 이동 피봇(결과물은 Getting Over It에 가까움) 후 우승 + 테이블/엑셀 레벨 에디터는 잼 이후 확장, 게임 철학(문명5·흥미로운 선택) — 각 문서에 반영 완료
