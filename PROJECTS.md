...
- 장비 투자 이력(사용자 확인): 홈서버 구매·운영, AI 서비스 종류별 유료 구독 비교, **로컬 LLM용 NVIDIA DGX Spark 구매** — "필요하면 장비에도 투자하며 탐구"의 근거 목록 (면접 답변 재료)
...
### Dangling (develop 브랜치) — 이력서의 "Creavas" (빙벽 등반 생존 게임, 🏆 학생 주도 게임잼 우승작)
- Unreal Engine 프로젝트 `Dangling_Climb`, 687개 파일, 38커밋, 마지막 커밋 2025-06-29
- ※ `master` 브랜치는 빈 껍데기(커밋 1개)였고, 실제 내용은 `develop` 브랜치에 있음
- 개발 기간 2025.06.28~29 (1박 2일 게임잼), Unreal Engine 5.6 + Blueprint, **학생 주도 게임잼(Dangling) 우승** — 여러 학과 학생들이 자발적으로 연 행사
