...
- **컨셉**: "얼음"을 주제로 빙벽을 등반하며 체온(체력)이 소진되기 전 목표 높이에 도달하는 'Getting Over It' 방식 발사형 이동 생존 게임
- **결정적 피봇(자소서 확인)**: 초기 기획은 단순 플랫포머 → 프로토타이핑 후 "너무 단순" 판단 → 'Getting Over It' 방식 발사형 이동으로 전환하며 게임성 개선, 우승. 참가팀 중 유일한 언리얼 사용 팀
- **담당 역할**: 게임 시스템 개발
  - 아이템 시스템: 핫팩(체온 손실 방지), 피켈(붙잡기 포인트 생성)
  - 체온 시스템: 시간 경과에 따라 체력 100에서 점진적 감소
  - ※ 데이터 테이블·엑셀 기반 레벨 에디터는 **잼 이후 확장 작업**으로 확인됨(자소서) — 이력서 표현과 달라 자소서 기준으로 정정
...
- **사용자 제공 자소서(2025)** → 추가 확보 정보: 산업기능요원 복무, 후반기 PM 업무(시즌 이벤트·상품 패키지·NFT 펫 리뉴얼), 밤샘 거래버그 픽스 에피소드, 3인 벽돌깨기 프로젝트(5~12세 타겟, SDK 추상화 — `G:\HDD_Backup\Git\BBF_Client`/`MFB_Client` 백업과 연결 추정), 원정대는 "탐사" 시스템, Creavas는 플랫포머→Getting Over It 방식 피봇 후 우승 + 테이블/엑셀 레벨 에디터는 잼 이후 확장, 게임 철학(문명5·흥미로운 선택) — 각 문서에 반영 완료
