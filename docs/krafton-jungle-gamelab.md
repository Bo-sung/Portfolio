...
| # | 프로젝트 | 무엇을 | 역할 | 핵심 포인트 |
| --- | --- | --- | --- | --- |
| 1 | **Creavas** (팀 4인, 무박 2일) | 빙벽 등반 생존 게임 | 애니메이션 제외 구현 전반 | 🏆 게임잼 우승, 프로토타입 후 방향 피봇, 유일한 UE 팀 |
...
| 3 | **BattleShip** (개인, 완성) | 3단 분산 게임 서버 | 단독 | 커스텀 TCP 프로토콜, 세션 프로세스 동적 spawn |
| 4 | **AI 파이프라인 툴 연작** (개인) | 개발 생산성 도구 5종 | 단독 | 3D→픽셀아트, 3D→ControlNet, HTML→UGUI, 에이전트 관리 |
...
| | |
| --- | --- |
| 기간 | 2025.06.28 ~ 29 (무박 2일 학생 주도 게임잼) |
| 역할 | 게임 시스템 개발 |
...
- **결정적 피봇**: 초기 기획은 빙벽 구간에서 시점이 전환되는 러너류 → **기획 구체화 단계**에서 "너무 단순" 판단 → 핵심 키워드(빙벽 등반)만 남기고 **점프킹을 모티브로 한 발사형 이동으로 과감히 전환**(결과물은 Getting Over It에 가까운 감각) — 게임성이 획기적으로 개선, 우승으로 이어짐
- 담당 구현: **발사형 이동 조작, 맵툴 제작·적용을 포함해 애니메이션 블루프린트를 제외한 구현 전반** — 아이템(핫팩·피켈)·체온 시스템, 난이도 구성 포함
- **에디터까지 잼 안에서**: 확장성을 고려한 데이터 테이블 + 엑셀 기반 레벨 에디터를 잼 도중 구현 — 기획자가 직접 맵을 만들며 스테이지를 빠르게 늘리는 구조
...
## 3. BattleShip — 3단 분산 게임 서버 (개인, 완성)
