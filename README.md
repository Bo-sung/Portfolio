...
"얼음" 주제 게임잼에서 'Getting Over It' 방식 발사형 이동의 빙벽 등반 생존 게임으로 우승했습니다. 스테이지별 난이도·아이템 데이터 테이블화, 빠른 스테이지 제작용 맵툴, 아이템(핫팩/피켈)·체온 시스템을 담당 — 1박 2일 안에 콘텐츠를 빠르게 찍어낼 수 있는 파이프라인을 만드는 데 집중했습니다.
...
> 비공개(NCloud SourceCommit) 프로젝트: UE5 익스트랙션 슈터 **MechTarkov**(기획 문서 포함), Unity VR/AR 프로젝트 **VRAR_Final**, Papago/ML Kit 폴백 구조의 AR 번역 앱 **MRTranslator("폴리글롯")** — 공개 준비 중
